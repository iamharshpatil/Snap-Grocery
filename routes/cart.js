...
{
 try{ let cart=await cartModel.findOne({user:req.session.passport.user}).populate("products");
...
{
  try { let cart=await cartModel.findOne({user:req.session.passport.user});
